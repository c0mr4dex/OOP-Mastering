...
void add_vector(vector <int> &vec);
void mean_vector (const vector<int> &vec);
void smallest_vector (const vector<int> &vec);
void largest_vector (const vector<int> &vec);
void display_end();
void display_other();
...
int main() {
    ...
    do {
        ...
        if (selection == 'P' || selection == 'p') {
          ...
          smallest_vector(numbers);
        } else if (selection == 'L' || selection == 'l') {
          largest_vector(numbers);
...
void mean_vector (const vector<int> &vec){
   if (vec.size() == 0)
        ...
        for (auto num: vec)
        total += num;
        cout << "The mean is : " << static_cast<double>(total)/vec.size() << endl;
    }
}
void smallest_vector (const vector<int> &vec){
      if (vec.size() == 0) 
